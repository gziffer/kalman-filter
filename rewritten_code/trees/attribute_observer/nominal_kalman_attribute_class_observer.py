...
class NominalKalmanAttributeClassObserver(AttributeClassObserver):
    """ Class for observing the class data distribution for a nominal attribute using Kalman Filtering.
    This observer monitors the class distribution of a given attribute.
    Used in naive Bayes and decision trees to monitor data statistics on leaves.

    """

    def __init__(self, q=10., r=2000.0):
        super().__init__()
    ...
    # function changed to handle kalman filtering output
    def observe_attribute_class(self, att_val, class_val, weight):
        if att_val is None:
            ...
            try:
                value_t0, p_t0 = self._att_val_dist_per_class[class_val][att_val]
                K = p_t0 / (p_t0 + self._r)
                value_t1 = value_t0 + K * (weight - value_t0)
                p_t1 = p_t0 * (1.0 - K) + self._q

                self._att_val_dist_per_class[class_val][att_val] = (value_t1, p_t1)

            except KeyError:
                self._att_val_dist_per_class[class_val][att_val] = (0.0, self._p)
                self._att_val_dist_per_class[class_val] = dict(sorted(self._att_val_dist_per_class[class_val].items()))

            for key in self._att_val_dist_per_class[class_val].keys():
                if key != att_val:
                    value_t0, p_t0 = self._att_val_dist_per_class[class_val][key]
                    K = p_t0 / (p_t0 + self._r)
                    value_t1 = value_t0 + K * (0.0 - value_t0)
                    p_t1 = p_t0 * (1.0 - K) + self._q

                    self._att_val_dist_per_class[class_val][key] = (value_t1, p_t1)
    ...
    def probability_of_attribute_value_given_class(self, att_val, class_val):
        ...
        if obs is not None:
            value = obs[att_val][0] if att_val in obs else 0.0
            return value
        return 0.0
