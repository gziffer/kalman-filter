...
from rewritten_code.trees.nodes.learning_node_nb_kalman import LearningNodeNBKalman  # learning node with Kalman Filter
...
class HoeffdingTreeClassifier(BaseSKMObject, ClassifierMixin):
    ...
    def leaf_prediction(self, leaf_prediction):
        if leaf_prediction != self._MAJORITY_CLASS and leaf_prediction != self._NAIVE_BAYES \
                and leaf_prediction != self._NAIVE_BAYES_ADAPTIVE and leaf_prediction != self._NAIVE_BAYES_KALMAN:
    ...
    def _new_learning_node(self, initial_class_observations=None, is_active_node=True):
        ...
        if is_active_node:
            ...
            elif self._leaf_prediction == self._NAIVE_BAYES_KALMAN:
                return LearningNodeNBKalman(initial_class_observations)  # classes added to include kalman at the leaf
